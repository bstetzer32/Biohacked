...
def parq_fail(form, field):
    answer = field.data
    if answer == 'yes':
        raise ValidationError("You answered yes to one of the first seven" +
                              "questions. In order to ensure your health and" +
                              " safety and in accordance with the National" +
                              "Academy of Sports Medicine's scope of " +
                              "practice for certified personal trainers, " +
                              "we cannot provide a workout to you" +
                              " at this time.")
...
class QuestionnaireForm(FlaskForm):
    ...
    barbell = StringField('barbell', validators=[DataRequired()])
    dumbbell = StringField('dumbbell', validators=[DataRequired()])
    cable = StringField('cable', validators=[DataRequired()])
    lever = StringField('lever', validators=[DataRequired()])
